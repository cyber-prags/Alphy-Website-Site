    ...
    <AppShell>
      ...
      <div className="mb-6 flex items-end justify-between gap-4 flex-wrap">
        ...
        <button onClick={() => setNewDealOpen(true)}
          className="text-[12.5px] font-semibold h-9 px-4 rounded-lg bg-ink text-white inline-flex items-center gap-1.5 hover:bg-ink-2 shadow-[0_4px_12px_-4px_rgba(28,40,64,0.2)]">
      ...
      <div className={`grid gap-4 ${filterRailOpen ? "grid-cols-[260px_1fr]" : "grid-cols-1"}`}>
        ...
        <div className="card overflow-x-auto">
          <table className="w-full table-fixed" style={{ minWidth: 1100 }}>
            <colgroup>
              <col style={{ width: 36 }} />
              {visibleFields.has("name") && <col style={{ width: "20%" }} />}
              {visibleFields.has("owner") && <col style={{ width: "11%" }} />}
              {visibleFields.has("stage") && <col style={{ width: "12%" }} />}
              {visibleFields.has("amount") && <col style={{ width: "8%" }} />}
              {visibleFields.has("forecast") && <col style={{ width: "10%" }} />}
              {visibleFields.has("closeDate") && <col style={{ width: "11%" }} />}
              {visibleFields.has("meddpicc") && <col style={{ width: "14%" }} />}
              {visibleFields.has("nextStep") && <col style={{ width: "14%" }} />}
            </colgroup>
            <thead className="bg-bg-deep">
              <tr>
                <th className="px-2 py-2"></th>
                {ALL_FIELDS.filter((f) => visibleFields.has(f.id as string)).map((f) => (
                  <th key={f.id as string} className="text-left px-3 py-2 text-[10.5px] font-semibold text-muted uppercase tracking-wider whitespace-nowrap">
                    {f.label}
                  </th>
                ))}
              </tr>
